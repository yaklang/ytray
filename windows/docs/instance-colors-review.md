...
- MSTest：98 通过，1 个已有的联网诊断测试默认跳过；无失败。
...
普通 Chrome 现在先检查实际插件加载状态，通过新 CDP 接口补载，成功后直接进入页面；只有实际失败才显示“不加载插件并启动”或取消。全局插件默认设置不变，历史恢复保留 Profile。代理认证插件同样可以通过新接口加载，加载失败时不能静默丢弃认证。

| 浅色确认 | 深色确认 |
| --- | --- |
| ![浅色兼容提示](images/chrome-plugin-light.png) | ![深色兼容提示](images/chrome-plugin-dark.png) |

本地 Chrome 152 已验证直连、自定义、历史恢复的真实插件功能，及 HTTP 407 代理认证。明暗弹窗在实际加载失败后显示，取消清理未完成的实例且保留旧历史；确认继续则重启为无插件实例，原有默认设置不变。详见 [接口验证说明](chrome-extension-api-review.md)。
