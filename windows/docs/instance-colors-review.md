...
- MSTest：91 通过，1 个已有的联网诊断测试默认跳过；无失败。
...
普通 Chrome 启动时选中了本地插件，现在弹出完整的兼容性说明，允许用户选择“不加载插件并启动”或取消。全局插件默认设置不变，历史恢复使用实际启动的插件列表。带认证信息的代理仍提示换用兼容浏览器，不能静默丢弃认证。

| 浅色确认 | 深色确认 |
| --- | --- |
| ![浅色兼容提示](images/chrome-plugin-light.png) | ![深色兼容提示](images/chrome-plugin-dark.png) |

本地通过真实小组件按钮和模态弹窗启动 Chrome 152，验证直连、HTTP 代理、自定义和历史恢复均能就绪，CDP 标题和截图正常，实例中插件列表为空，全局插件设置保存并重载后仍启用。代理测试使用本地地址与内嵌页面验证配置保留，不代表外部代理连通性验证。明暗弹窗及认证说明均检查全文换行；取消后小组件保留、按钮恢复、不产生错误提示或新 Profile。
