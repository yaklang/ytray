...
final class BrowserLauncherTests: XCTestCase {
    ...
    func testBrowserIdentityColorsAreStableAndCycleAfterPalette() {
        XCTAssertEqual(BrowserIdentityColor.color(for: "A"), .init(red: 242, green: 98, blue: 21))
        XCTAssertEqual(BrowserIdentityColor.color(for: "B"), .init(red: 37, green: 99, blue: 235))
        XCTAssertEqual(BrowserIdentityColor.color(for: "C"), .init(red: 22, green: 163, blue: 74))
        XCTAssertEqual(BrowserIdentityColor.color(for: "G"), BrowserIdentityColor.color(for: "A"))
        XCTAssertEqual(BrowserIdentityColor.color(for: "AA"), BrowserIdentityColor.palette[2])
    }

    func testNewProfileCanInstallGeneratedThemeWithoutAffectingRestores() throws {
        let settings = LaunchSettings()
        let color = BrowserIdentityColor.color(for: "B")
        let firstLaunch = try BrowserLauncher.buildArguments(
            mode: .quick,
            settings: settings,
            profilePath: "/tmp/new-profile",
            debugPort: 9441,
            plugins: [],
            identityColor: color
        )
        XCTAssertTrue(firstLaunch.contains("--install-autogenerated-theme=37,99,235"))

        let restored = try BrowserLauncher.buildArguments(
            mode: .quick,
            settings: settings,
            profilePath: "/tmp/restored-profile",
            debugPort: 9442,
            plugins: [],
            restoreLastSession: true
        )
        XCTAssertFalse(restored.contains(where: { $0.hasPrefix("--install-autogenerated-theme") }))
    }

    func testInstanceColorSettingDefaultsOnAndRoundTripsOff() throws {
        let legacy = try JSONDecoder().decode(LaunchSettings.self, from: Data("{}".utf8))
        XCTAssertTrue(legacy.colorizeBrowserInstances)

        var disabled = LaunchSettings()
        disabled.colorizeBrowserInstances = false
        let restored = try JSONDecoder().decode(
            LaunchSettings.self,
            from: JSONEncoder().encode(disabled)
        )
        XCTAssertFalse(restored.colorizeBrowserInstances)
        XCTAssertEqual(restored.configurationVersion, LaunchSettings.currentConfigurationVersion)
    }

    func testInstanceColorSettingControlsResolvedBrowserColor() {
        var settings = LaunchSettings()
        XCTAssertEqual(
            BrowserLauncher.resolvedIdentityColor(badge: "C", settings: settings),
            BrowserIdentityColor.color(for: "C")
        )
        settings.colorizeBrowserInstances = false
        XCTAssertNil(BrowserLauncher.resolvedIdentityColor(badge: "C", settings: settings))
    }
    ...
    func testCustomFlagsCannotOverrideIsolationBoundary() {
        ...
        settings.additionalFlags = "--install-autogenerated-theme=1,2,3"
        XCTAssertThrowsError(try BrowserLauncher.buildArguments(
            mode: .custom,
            settings: settings,
            profilePath: "/tmp/isolated",
